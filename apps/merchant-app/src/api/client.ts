...
export function apiErrorMessage(err: unknown): string {
  ...
  if (!err.response) {
    return 'Cannot reach API. Start Docker Desktop + postgres, then backend on port 3000.';
  }
...
api.interceptors.request.use((c) => {
  const t = localStorage.getItem('rampx_merchant_token');
  if (t) c.headers.Authorization = `Bearer ${t}`;
  return c;
});
api.interceptors.response.use(r => r, e => {
  if (e.response?.status === 401) { localStorage.removeItem('rampx_merchant_token'); window.location.href = '/login'; }
